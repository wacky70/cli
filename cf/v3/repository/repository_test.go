...
import (
	...
	repositoryfakes "github.com/cloudfoundry/cli/cf/v3/repository/fakes"
...
var _ = Describe("Repository", func() {
	var (
		...
		tokenHandler *repositoryfakes.FakeTokenHandler
	)
	...
	BeforeEach(func() {
		...
		r = repository.NewRepository(ccClient, tokenHandler)
		tokenHandler.DoStub = func(cb func() ([]byte, error)) ([]byte, error) {
			return cb()
		}
	})
	...
	Describe("GetApplications", func() {
		It("tries to get applications from CC with a token handler", func() {
			tokenHandler.DoStub = func(cb func() ([]byte, error)) ([]byte, error) {
				Expect(ccClient.GetApplicationsCallCount()).To(BeZero())
				cb()
			...
			r.GetApplications()
			Expect(tokenHandler.DoCallCount()).To(Equal(1))
		})
	...
	Describe("GetProcesses", func() {
		It("tries to get processes from CC with a token handler", func() {
			tokenHandler.DoStub = func(cb func() ([]byte, error)) ([]byte, error) {
				Expect(ccClient.GetResourcesCallCount()).To(BeZero())
				cb()
